...
cbuffer P : register(b0) {
    ...
    float  fovTan; float emissive; float growth; float pad;
};
...
[numthreads(8,8,1)] void CSMain(uint3 id:SV_DispatchThreadID){
    ...
    float2 ndc=float2((id.x+0.5)/W*2-1, 1-(id.y+0.5)/H*2);
    ...
    [loop] for(int s=0;s<192;++s){ p=ro+rd*t; float3 cc; float d=mapD(p,cc);
        if(d<0.0012){ hit=true; hitcol=cc; break; } t+=d; if(t>7.0) break; }
    if(hit && emissive>0.0){
        ...
        [loop] for(int s=0;s<40;++s){ float d=mapOnly(p+n*0.01+L*tt); sh=min(sh, 14.0*d/tt); if(d<0.001||tt>3.0) break; tt+=max(d,0.012); }
        sh=saturate(sh);
        float ao=0, sca=1;
        [unroll] for(int k=1;k<=5;++k){ float hh=0.015*k; float dd=mapOnly(p+n*hh); ao+=(hh-dd)*sca; sca*=0.65; }
    ...
    Hdr[id.xy]=float4(col,1.0);   // raw HDR
}
